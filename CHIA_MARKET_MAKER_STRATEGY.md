...
- **Core constraint (historical -- see §8 policy update)**: "never sell at a loss" was built into all algorithms; since v0.10.10 it is the `no_loss_floor_mode` operator dial (strict/aging/off), currently `off`
...
### CORE RULE: NEVER SELL AT A LOSS (now an operator dial)
...
Under `strict`, the original rule holds: it invalidates stop losses, timeout exits, and forced EOD closes, and all risk management works around the constraint. Under `aging`, losses are realized on a configured schedule. Under `off`, asks price at the market and the sections below that assume the absolute rule should be read accordingly.
...
| Underwater position | cost_basis > market | mode `strict`: hold, only offer above cost_basis; `aging`: floor decays with age; `off`: offer at market |
...
*CONSTRAINT (mode-dependent since v0.10.10): all strategies enforce the `no_loss_floor_mode` policy -- absolute only under `strict`*
