...
- **Not covered by a C++ test:** the wiring in `engine.cpp` (S36). `tests/test_stop_keep_wiring.py` pins it over the source text -- lint-class, disclosed there.
- **Status:** `[x]` this PR.
...
- **That holds for `strategy.ttl_cancel_mode: age` only** (the default, and every build before PR #164 added the key). Under `expire`, `execution::age_limit_cancel_applies(expire_mode, verified_max_time)` is false for any offer carrying a verified expiry, so the hard-TTL cancel is skipped and the offer rests until `retire_expired_offers` frees it -- after its own `max_time`, plus the reorg depth. The bound a kept book really has is then the expiry (24 h at the live `offer_expiry_secs: 86400`), never the hard TTL, and the operator note says both. `KeptBookAfterRestart.UnderExpireModeTheBoundIsTheExpiryAndItOutlastsTheHardTtl` pins that the expiry is the longer of the two at every configuration the startup floor accepts.
