...
- **Open, each needs its own decision or PR:**
  ...
  - **(f) A per-heartbeat skip sets no gauge reason.** `xop_posting_gated{reason=wallet_circuit}` reflects only an OPEN breaker, while a skip also bypasses the GUI-pause and breaker-pause TTL sweeps, so `gui/services/venue_control.py` `_NO_DRAIN_GATES` can read "draining" for a heartbeat that did not drain.
