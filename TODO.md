...
### S76: State carries no on-chain expiry per offer, `engine.shutdown_offers` is read at startup only, a signal can cut a cycle short, and no stop policy can be guaranteed at an OS session end
...
- **Found while building S74, 2026-09-20.** Four limits, each stated where it bites rather than hidden:
  ...
  - **A keep stop delivered by a SIGNAL can cut a heartbeat cycle short** (review of PR #165). `shutdown.flag` is read between cycles, so a GUI stop never lands inside one; Ctrl+C or SIGTERM can, while the cycle is suspended in `create_offer`. The keep continuation then runs to `ioc_.stop()` without suspending, the cycle never resumes, and the wallet may hold one offer with no State entry, no `offer_log` row and no Dexie listing. The next start meets it as an orphan and re-prices or cancels it, which is what a hard kill has always left. The keep report says so (`heartbeat_in_flight_`); it does not wait the cycle out, because the cycle would go on posting and cancelling under a stop meant to be quiet. A fix is to make `post_quotes`' abort predicate honour a keep stop and then drain the one in-flight create, recording its result, before the report.
