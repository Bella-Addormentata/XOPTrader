...
- **Files:** `cpp/include/xop/util/stop_offers_policy.hpp` (the policy and its table), `cpp/include/xop/util/shutdown_flag.hpp` (the `offers=` line), `cpp/include/xop/execution/kept_book.hpp` (the keep line), `cpp/src/engine.cpp` (`Engine::shutdown`, `report_offers_kept_on_stop`, `watchdog_cancel_book`, `evaluate_shutdown_flag`), `cpp/src/config.cpp` (`engine.shutdown_offers`), `gui/stop_offers.py`, `gui/widgets/stop_engine_dialog.py`, `gui/widgets/main_window.py`, `gui/services/engine_bridge.py`, `gui/widgets/settings.py`
...
- **Found while building S74, 2026-09-20.** Four limits, each stated where it bites rather than hidden:
  ...
  - **A keep stop delivered by a SIGNAL can land inside a heartbeat cycle** (review of PR #165). `shutdown.flag` is read between cycles, so a GUI stop never does; Ctrl+C or SIGTERM can, while Step 8 is suspended in `post_quotes`. FIXED for the case that mattered: the keep stop now waits, bounded at 60 s (`util::keep_stop_drain_step`), while the one offer-creating call is in flight, so a `create_offer` the wallet is still answering lands in State and `offer_log` instead of becoming an orphan -- which `evaluate_orphan` CANCELS at the next boot when it is older than `orphan_max_adopt_age_blocks`, adversely priced, or on a pair with no mid price. What remains open: (a) `post_quotes` finishes the REST of that pair's tiers during the wait, because `OfferManager` has no "stop creating, cancel nothing" predicate (its abort predicate cancels the offer that landed late, which a keep stop must not do); (b) a create still unanswered at the budget is abandoned, and the log says so at error; (c) a cancel or a take the cycle was awaiting is still cut -- the wallet settles it alone, boot adopts a PENDING_CANCEL record as `cancel_pending`, but a take completed that way is booked nowhere, exactly as after a hard kill.
