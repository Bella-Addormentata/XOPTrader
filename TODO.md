...
### S76: State carries no on-chain expiry per offer, `engine.shutdown_offers` is read at startup only, and no stop policy can be guaranteed at an OS session end
...
- **Found while building S74, 2026-09-20.** Three limits, each stated where it bites rather than hidden:
