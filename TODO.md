...
- **Files:** `cpp/include/xop/execution/offer_expiry.hpp` (`age_limit_cancel_applies`, `trade_record_max_time`, `expired_beyond_safety`, `decide_expired_retire`), `cpp/src/execution/offer_manager.cpp` (`classify_tier_staleness`, `cancel_stale`, `retire_expired_offers`, `detect_fills` backfill), `cpp/src/engine.cpp` (Step 8, below the sync gate), `cpp/include/xop/rpc/wallet_requests.hpp` (`get_timestamp_for_height`)
...
- **Done (default OFF):** `strategy.ttl_cancel_mode: expire` skips the hard-TTL cancel and the stuck pass for an offer with a verified expiry, and retires it with a free local cancel once the wallet's chain clock (`get_timestamp_for_height` at `get_height_info`'s finished-sync height) is 600 s past `max_time`, the wallet still says PENDING_ACCEPT, and its record repeats the tracked `max_time`. The offer_log row is still completed only by the wallet verdict (`expired_onchain` is a submit-time reason).
- **Not done:** `offer_log` has no expiry column, so a restart recovers each offer's expiry from the first `get_offer` poll (Step 2 runs before Step 8); an offer the poll backoff skips on that first heartbeat, and is already past the hard TTL, is cancelled the old way. The stopped-engine sweep (`ttl_while_stopped`) is deliberately unchanged. #150's startup floor (`offer_expiry_secs` > hard TTL x 2) is also unchanged, although under `expire` the hard TTL no longer applies to the offers it protects: it keeps the expiry at ~23 h or more at `offer_ttl_blocks: 400`, and an offer the market has drifted away from rests that long when `price_cancel_mode: margin` is on as well. Relaxing the floor in `expire` mode trades that for a larger share of each offer's life spent expired-but-still-tracked (600 s per life), and is the operator's call. The GUI pairs table sizes its resting-offer window from the expiry in this mode (`book_window_ttl_blocks`); no other GUI surface knows the three keys.
...
- **Done (default OFF):** `strategy.price_cancel_mode: margin` cancels for price only when a fill at the resting price would earn less than `price_cancel_edge_retain` (default 0.5) x Step 7's own floor against Step 7's own centre, both threaded from `PairCycleState`; crossed still outranks everything, `kMinRefreshAgeBlocks` still applies, the anchor override is off in this mode, and a missing reference falls back to the deviation zones.
