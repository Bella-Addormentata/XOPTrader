...
- **What the PR does:** `strategy.offer_min_input_coin_frac` (default 0.01) keeps dust out of the selection, so a CAT-funded offer needs at most 100 inputs plus a fee coin, and a refusal logs one `[dexie-too-many-inputs]` warning. It adds no auto-cancel.
