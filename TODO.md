...
- **Status:** `[~]` -- the breaker now reads the transport evidence `rpc_post` records for every call: one unanswered transport failure skips the rest of that heartbeat's wallet work, three consecutive failures open the breaker. No wallet call site in `step_manage_offers` can be reached after an unanswered transport failure without passing a checkpoint first; the checkpoints before the exposure-floor and both-sides-suppressed cancels, in the force-delete loop, before the XCH fee-reserve query and around the on-chain reconciliation calls were added after a verification pass found those sites unchecked. The startup scan builds the wallet-ID map first, defers rather than shrinking to XCH, and releases the map afterwards, so the process's map is still first built after the wallet sync wait. The pure rules, `rpc_post`'s CurlFailed recording and its not-open refusal are tested and mutation-checked; its HttpError, MalformedBody, success=false and Ok recordings (no test server) and the engine and OfferManager wiring (S36) are not.
