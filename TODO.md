...
### S38: TibetSwap is halted, but we still poll it every 60s -- and its fee still sets our quoting floor
- **Files:** `cpp/include/xop/config.hpp` (`TibetSwapConfig::enabled`, `arbitrage.tibetswap_fee_bps`), `cpp/src/engine.cpp:250` (client construction), `:3315-3350` (poll loop), `:7098-7101` (the width floor)
- **Observed 2026-09-01:** `TibetSwap returned no pools -- keeping cached reserves` appears **1,738 times** across the six live log rotations (206/236/300/345/337/314), roughly once a minute, with **no successful pool fetch anywhere in the retained window** -- only the `TibetSwap AMM reference enabled at ...` startup lines. The venue has been down since the security incident (see the 2026-08-24 note in the operational memory).
- **Two SEPARATE settings, and conflating them is the trap:**
  1. `tibetswap.enabled` (default **true**, `config.hpp`). There is **no `tibetswap:` section in `config.yaml`** -- only in `config.example.yaml:653` -- so the default binds and we are polling a dead API. Disabling costs nothing: `is_open()` gates the poll, the AMM leg carries its own freshness taper (`market_data.cpp:1352`), and `mid_gate`'s anchor chain falls through `Amm` to `FairValue`/`Peg` on its own.
  2. `arbitrage.tibetswap_fee_bps: 70` (`config.yaml:290`). **Disabling the API does NOT change this**, and this is the one that matters: `engine.cpp:7098-7101` sets `quote_base_floor_bps = max(min_profit_margin_bps 50, tibetswap_fee_bps 70) = 70 bps` of HALF-spread. Measured, the market's **full** spread on XCH/DBX is 8.0 bps in 86% of observations -- so this floor puts us 17-35x outside the entire book, the competitive anchor is overwritten in 99.9% of cycles, and score-10 (at-touch) offers fill 5.62% against 0.95% for everything else.
- **The open question this raises, and it is NOT to be answered by flipping a value:** the fee's presence in the QUOTING floor (as opposed to in arbitrage detection, `arbitrage.cpp:482,551`, where it plainly belongs) reads as "do not quote inside what it would cost an arbitrageur to pick us off and unwind on TibetSwap". If TibetSwap is halted that unwind route does not exist, so the floor may currently be pricing a hazard that is not there. But the outage is temporary, the floor is a safety control, and the comment at `engine.cpp:7084-7097` does not actually state the rationale -- it explains the sigma and margin terms only. **Establish the rationale before touching it.**
- **Status:** `[ ]` -- OPEN, two independent actions:
  - **(a) Operator, low risk, do now:** add a `tibetswap:` section to `config.yaml` with `enabled: false` while the venue is down; revert when it returns. Stops 1,440 failed polls a day and the warning spam that hides real Step 1 problems. Requires an engine restart. **Do not edit `config.yaml` while the GUI is running** -- it rewrites the file on Save.
  - **(b) Engineering, higher value, needs analysis first:** decide whether `tibetswap_fee_bps` belongs in the quoting floor at all, and if so whether it should be gated on the venue being reachable. This is the prerequisite for the biggest measured fill lever on XCH/DBX. See also S34/S36 -- any change to the floor lands in Step 8's blast radius and nothing in `cpp/tests` constructs an `Engine`.
- **Related:** the 2026-08-24 TibetSwap halt is already recorded in the operational memory; this entry is about the two config consequences that were never followed up.
