...
- **Status:** `[~]` -- BUILT, OFF. Staged enable is in the PR body: `cost_aware_estimate` first, then `controller_enabled` with `max_fee_mojos` raised (the startup log names the class the live 100M cannot get into a full mempool) and `daily_budget_mojos` raised from the live 10,000,000,000 to at least 30,327,171,874.
