...
- **It also poisons the books -- structurally, though not yet in fact.** `engine.cpp:11538` passes the CLAMPED `take_size` to `record_taker_fill`, which then derives the quote leg as `quote_mojos_for(base_mojos, price_mojos, ...)`. So both double-entry ledger legs are **synthesised from a size the RPC never saw**, and any take that consumed more than the cap would be recorded at the cap.
  - **Checked 2026-09-01: no corruption has occurred yet.** All 23 `crossed_book` rows in `taker_fills` settled BELOW the 5 XCH cap (max 3.271 XCH-equiv; per pair: XCH/wUSDC.b 3.271, XCH/BYC 2.000, XCH/DBX 1.290, BYC/wUSDC.b 0.000). The only rows at or above the cap are 44 `historical_backfill` entries, including all 6 exactly at 5e12, which do not come from this path. So `take_size == best_ask_size` in every recorded case and **no historical remediation is needed** -- the defect is purely forward-looking. That is a consequence of never having had the funds, not of the code being right.
...
- **Also missing on this path:** no pre-trade balance check, no backoff, no blacklist. One offer was re-attempted **50 times**.
- **Status:** `[ ]` -- OPEN, and this is the highest-value item in the file. It is the only finding that is simultaneously source-established, live-corroborated and DB-corroborated.
...
- **Status:** `[ ]` -- OPEN. Propagate, or return an explicit error type. Small change; add it to the fail-open family ledger.
