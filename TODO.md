...
- **`[ ]` STILL OPEN on this path:** no pre-trade balance check in 9c, no backoff, no blacklist. One offer was re-attempted **50 times**. The size filter removes the *historically observed* instance of the retry storm (the 208 retried attempts were the at-cap ones, now declined before both RPCs) but **the mechanism is unchanged**: a sub-cap crossed ask we cannot fund is re-selected every cycle it remains cheapest. The pre-trade balance check is the cheapest of the three and can be lifted from Step 9e, which has one.
- **Status:** `[~]` -- PARTIAL. The P0 half (the inert cap, on all three paths) is closed and the four ledger rows are now ADJUDICATED (corruption confirmed and quantified). Outstanding: the retry/backoff/blacklist half, the remediation entries, and the missing `ledger_entries` rows.
