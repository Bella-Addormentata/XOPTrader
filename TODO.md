...
- **Compounding:** the same print marked the book up an instant before the equity peak was sampled, contributing to the `$262.11` peak that then tripped the max-drawdown breaker at 2026-08-22T09:43:34Z. One tick both halted posting and caused the pause. The breaker's peak is in-memory and re-anchors on restart (`cpp/include/xop/engine.hpp`, `peak_equity_hwm_usd_`).
