...
- **It also poisons the books.** `record_taker_fill` (`engine.cpp:14291`) derives the quote leg as `quote_mojos_for(base_mojos, price_mojos, ...)`, so the entire `taker_fills` row and both double-entry ledger legs are **synthesised from a size the RPC never saw**. If a take ever settles at a size other than the one we assumed, P&L and inventory are silently wrong -- and nothing would flag it.
