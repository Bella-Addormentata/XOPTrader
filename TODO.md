...
- **The fee in the quoting floor: RESOLVED, leave it alone.** The rationale is real and was found in the introducing commit `679a934` (2026-04-04), verbatim: *"The TibetSwap fee creates a natural arbitrage boundary - any offer tighter than ~70 bps can be profitably arbed."* So the term is not arbitrary. Two caveats worth recording: it was never reviewed (no design note, review or strategy doc discusses the quoting use), and its scope was silently widened -- in `679a934` it floored only the competitive cap, not the whole ladder.
  ...
  - **Unresolved and material:** post-fill drift conflates informed pickoff with our own mid being mislocated at post time. Both give identical P&L so "do not tighten" holds either way -- but if it is mislocation the fix is a better mid, not a wider floor. There is direct evidence mislocation is present (April DBX at-touch fills sat 258-455 bps below the third-party book mid while 7-205 bps above our own).
...
- **Neither value is a coincidence; both are the analytic maximum, so the controllers have no authority left.**
  - Spread PID: total authority is `kp*target + ki*integral_max` = `0.8*0.10 + 0.05*2.0` = **0.18**, so `mult` bottoms at `1 - 0.18 = 0.820` -- exactly the logged value. The derivative term is bounded by `kd * ema_alpha` = `0.2*0.02` = 0.004, so the true reachable minimum is 0.816.
  - **`pid_min_mult: 0.7` (`config.yaml:216`) is unreachable dead config.** The gains cannot produce an output large enough to approach it. It is set explicitly in `config.yaml`, so an operator would reasonably believe it is a live knob; it is not, and no test binds it.
  - Competitiveness PID: `config.yaml:220` sets `comp_pid_target_fill_rate: 0.15`, **three times the `0.05` default at `config.hpp:1263`**. That gives `8.0*0.15 + 0.5*4.0` = **3.20**, raw offset `-3.20`, rounding to `-3` -- precisely `comp_pid_min_offset`. (`comp_pid_warmup_blocks` diverges the same way: 5 in the YAML, 50 in the header. And `pid_integral_max` is absent from `config.yaml` entirely, so the 2.0 default binds while every sibling knob is explicit -- easy to misread as tuned.)
- **The consequence nobody signed off on:** `kBaseCompetitivenessScore` is 3 (1 for stablecoin pairs, `engine.cpp:10577-10578`); `3 + (-3)` clamps to **`effective=0`**, i.e. the Step 8 competitiveness gate admits every tier regardless of score. A safety gate has been fully open for the entire retained window because a controller was doing exactly what it was told. Confirmed in the logs: `comp gate base=3 PID-offset=-3 -> effective=0`.
...
- **Status:** `[ ]` -- OPEN. Actions in value order:
  ...
  - **(b) Decide the competitiveness gate deliberately.** Either `effective=0` is correct -- in which case set the base to 0 and delete the pretence of a gate -- or it is not, in which case bound `comp_pid_min_offset` so feedback cannot fully open a safety gate. Right now the answer is set by a rounding accident in a controller nobody is watching.
