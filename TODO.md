...
- **Done (default OFF):** `strategy.ttl_cancel_mode: expire` skips the hard-TTL cancel and the stuck pass for an offer with a verified expiry, and retires it with a free local cancel once the wallet's chain clock, read 32 blocks BELOW `get_height_info`'s finished-sync height (`get_timestamp_for_height`), is at or past `max_time`, the wallet still says PENDING_ACCEPT, and its record repeats the tracked `max_time`. The depth is the reorg allowance: the first draft used a 600 s margin at the tip, which review on #164 showed the tip block alone can satisfy after a slow patch. The offer_log row is still completed only by the wallet verdict (`expired_onchain` is a submit-time reason).
