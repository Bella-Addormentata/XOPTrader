...
- **Status:** `[x]` — FIXED 2026-09-13 (branch `fix/dexie-ticker-bid-ask`). **This entry was right; its 2026-08-21 closure as NOT A DEFECT was not.** The closure reasoned that `get_ticker` already applied an invert+swap for Case A (04e96e1, 2026-04-03), so `parse_ticker_`'s labels were raw values that did not matter yet. The flip is correct -- but only for correctly labelled inputs, and Dexie's `prices.buy` is the token's ASK, `prices.sell` its BID. Proof, 2026-09-13: our own resting XCH/BYC bid (1.451 BYC per XCH) is listed as BYC `buy[0]` = 0.6891798759 = 1/1.451 under `xch`, and as XCH `sell[0]` = 1.451 under the BYC key. Every oriented ticker therefore had its sides exchanged, Case A (XCH/wUSDC.b, XCH/BYC, XCH/DBX) and Case B (BYC/wUSDC.b, listed under wUSDC.b's key -- markets are not all keyed by XCH) alike. The closure verified that an orientation step existed, never what `buy` means.
  The fix is this entry's relabel with the flip kept (`TickerData::best_bid` / `best_ask`); orientation is now the pure `orient_market_ticker()`, pinned by `cpp/tests/test_dexie_market_ticker.cpp` -- the regression test the closure asked for. Two corrections to the Issue text: the offer_manager ticker mid is a symmetric sum and was never affected, and OFI barely depends on the swap: both sides share one depth proxy, so exchanging the prices changes a step's size only when exactly one side moves while that proxy changes, and never a step's sign while the proxy is positive (a near-zero window sum can still tip either way). Round 2 of the same PR gated `get_fair_value_inputs` and the published `spread_bps` on `bbo_from_filtered_book`: read correctly, the raw ticker no longer looks crossed and would otherwise have reached both. Round 3 pinned the Case A/B search order, which decides `last` when Dexie lists a market under both keys. Found on the way: S52, S53, S54.
...
- **[2026-09-13] Escalation ladder IMPLEMENTED (branch `fix/cancel-pending-proof-and-escalation`, cancel-truth PR-A).** An accepted cancel now leaves the offer `cancel_pending` in offer_log until the wallet reports it terminal; `Engine::escalate_stuck_cancels` re-cancels SECURE with a raised fee only when the wallet reports the trade live AND the full node shows every maker coin unspent (`execution/cancel_escalation.hpp`), up to `strategy.cancel_escalation_max_attempts` (default 3, counted across restarts), then ONE `CancelUnresolved` alert; the Step 8 stuck summary logs on a change or every 32 blocks. Still open here: the `delete_unconfirmed_transactions + re-cancel` rung (prune_stuck_transactions can delete an escalation's own re-cancel), and the bulk-cancel mechanics (PR-B).
...
- **Status:** `[x]` -- RESOLVED (2026-09-03). Promoted `classify_cross_bbo` from shadow logging to active decision in Step 8, matching `classify_tier_staleness`. Updated selective refresh to detect unposted new tiers. Live verified on `XCH/BYC` with 2-sided quoting (bids @ 1.35-1.38, asks @ 2.07-2.23 BYC/XCH).
...
- **Status:** `[ ]` -- OPEN. Actions in value order:
  ...
  - **(b) The competitiveness gate: leave it OPEN. The DBX measurement decides DBX and does not transfer to BYC.**
    - **On XCH/DBX the gate is irrelevant.** Of offers scoring below the configured base of 3, **5 of 1,477 filled = 0.34%**, against 71/2,873 = 2.47% for score>=3 (April excluded -- its scores are a migration backfill). XCH/DBX is the only pair with `enabled: true`. Re-closing the gate would delete 1,477 postings to avoid an exposure realised five times in four months. **Do not close it.**
    - **On XCH/BYC the gate is LOAD-BEARING, which is a different statement.** Low-score offers there fill at **95/830 = 11.45%**, against its own score>=3 rate of 10.86% -- essentially the same. So the DBX argument ("closing it costs nothing because these never fill") is FALSE on BYC: closing it there would remove ~95 fills, not 5. Whatever is done with the gate on BYC has real consequence.
    - **CORRECTION 2026-09-02: an earlier revision of this entry said the decision "inverts" for BYC. That is wrong and the distinction matters.** The REASONING fails to transfer; the CONCLUSION does not flip. Both pairs end at "leave it open" -- DBX because the gate is decoration, BYC because there is no evidence closing it would help.
    - **The evidence that nearly changed the answer, and why it did not.** Fill rate says those offers fill, not that filling was good, and XCH/BYC carries the worst post-fill markout of any pair. Split by bucket (h=36 blocks, signed so positive = moved against us): low-score **mean +283.2 bps (n=87)** against high-score **+136.4 (n=355)** -- an apparent **2.1x** adverse selection and a clean argument for closing the gate on BYC. **It does not survive a robustness check.** Trimming 10% collapses the gap from 147 bps to about 15 (67.5 vs 52.9); low-score fills move against us slightly LESS often (40.2% vs 42.0%); the p90s are identical at 424.3; and a 20,000-shuffle permutation test on the mean gap gives **p = 0.052**. The entire difference is a handful of tail episodes -- the same concentration that forced the retraction of the +244 bps headline figure. **Not established. Do not act on it.**
    - **What would actually settle it:** a markout construction that does not depend on the published mid, which S43 showed is unreliable (clamped onto `best_bid` on 46.9% of ingests); or simply more fills. At n=87 with a tail-driven distribution this sample cannot distinguish a good fill from a bad one, in either direction.
    - **The honest change is not to the gate but to the SAWTOOTH.** The effective threshold is 3 for the first five ticks of every run and 0 thereafter (see the correction above), so gate behaviour is not reproducible across restarts. Either set the base deliberately in source, or bound `comp_pid_min_offset` to -2 (which binds through `effective_comp_pid_min_offset = max(configured, reachable)`).
...
### S47: the wallet breaker never opened on a stalled wallet -- observed live 2026-09-12
- **Files:** `cpp/include/xop/rpc/transport_evidence.hpp`, `cpp/include/xop/execution/wallet_circuit.hpp`, `cpp/src/rpc/chia_rpc.cpp` (`rpc_post`), `cpp/src/engine.cpp` (`Engine::wallet_step_may_run` and its call sites, the S46 sweep, the startup prune), `cpp/src/execution/offer_manager.cpp` (`detect_fills`), `cpp/include/xop/execution/stuck_prune_scope.hpp`
- **Observed.** Blocks 9284260, 9284302 and 9284313 took 156,409 / 170,195 / 172,851 ms: 10, 9 and 11 wallet calls in series, each exhausting ~15.5 s of libcurl retries (CURL 28). The breaker counted THROWS out of Steps 2 and 8; neither step throws for a stall (detect_fills swallows each get_offer failure, Step 8's sync check logs and returns), so it never opened in any of three stall windows. Separately, the startup stuck-transaction scan ran before the wallet-ID map existed and visited wallet 1 alone (23:24:25.792; map built 23:24:27.548).
- **Status:** `[~]` -- the breaker now reads the transport evidence `rpc_post` records for every call: one unanswered transport failure skips the rest of that heartbeat's wallet work, three consecutive failures open the breaker. Every wallet call site in `step_manage_offers` has a checkpoint of its own or directly follows a gate or an answered call; the checkpoints before the exposure-floor and both-sides-suppressed cancels, in the force-delete loop, before the XCH fee-reserve query and around the on-chain reconciliation calls were added after a verification pass found those sites unchecked. The transport streak is shared with every other wallet caller, so a failure another coroutine records while Step 8 is suspended is seen only at Step 8's next checkpoint. The startup scan builds the wallet-ID map first, defers rather than shrinking to XCH, and releases the map afterwards, so the process's map is still first built after the wallet sync wait. The pure rules, `rpc_post`'s CurlFailed recording and its not-open refusal are tested and mutation-checked; its HttpError, MalformedBody, success=false and Ok recordings (no test server) and the engine and OfferManager wiring (S36) are not.
- **Open, each needs its own decision or PR:**
  - **(a) `std::system("chia stop wallet & chia start wallet")` blocks the single IO thread.** It runs inside `step_manage_offers` after `kWalletRestartThreshold{20}` unsynced answers, and the engine has one `ioc_.run()` thread. Run it off-thread behind an in-progress latch and let the transport gate absorb the restart window's failures. Its log line's "blocks x 9 s" is also stale against block cadence.
  - **(b) OPERATOR DECISION: should the `kForceDeletePendingBlocks` backstop honour an in-flight-cancel hold?** Deliberately NOT done. The hold proposed during review keyed on `cancel_pending` and S46 intent ids, and both fire on wallet PENDING_CANCEL, which is not proof a cancel was broadcast (2026-08-29: 33 PENDING_CANCEL trades, 9 with a spent maker coin). After a restart it would have disabled prune and force-delete on the XCH wallet indefinitely. If a hold is wanted, key it on positive evidence (a cancel RPC this process saw succeed) with an expiry and a loud log, or make the delete bundle-aware in `stuck_tx_verdict.hpp`.
  - **(c) OPERATOR DECISION: untraded wallets are never pruned.** Wallet 2 (wUSDC.b; XCH/wUSDC.b and BYC/wUSDC.b are disabled) holds unconfirmed rows from 2026-08-29/30. Clean them up by hand, or scan every wallet in the map?
  - **(d) S46 reads wallet PENDING_CANCEL as "a cancel spend is in flight" and waits forever.** The five XCH/DBX intents it re-logs as waiting every heartbeat came from shutdown cancels the wallet REFUSED at 22:48 on 2026-09-12 ("Wallet needs to be fully synced"); a read-only wallet DB check during review found them at status 2 with sent=0, so nothing is in flight and nothing re-fires them. Needs node coin-spend verification and a secure re-cancel with a fee (the S14 ladder). Untracked PENDING_CANCEL trades are invisible to the engine altogether: startup_reconcile and reconcile_offers adopt only PENDING_ACCEPT.
  - **(e) Wallet calls still issued after a transport failure.** In `step_manage_offers` every call site has a checkpoint or directly follows a gate or an answered call, but a routine that is already running when the failure happens makes the rest of its calls: `selective_cancel` and `cancel_stale` (one secure cancel per remaining offer), `post_quotes` (its `create_offer` calls and XCH balance re-checks), `reconcile_offers`, `run_full_reconciliation` (`verify_pending_offer_coins`, then `reconcile_balances` per wallet), `emergency_cancel`'s fallback attempts, and the internals of steps gated only at entry (coin pool maintenance, `step_xch_recovery`, the drift corrector). Against a wallet that accepts the connection and then hangs, each call costs ~123.5 s, so a stall that begins inside `selective_cancel` with five or more offers to cancel reaches 617.5 s on its own, past `watchdog_stall_seconds` (600). A local transport mark like the one in `detect_fills` would bound each such loop to one doomed call. Also ungated: the startup path (startup_reconcile, the startup coin pool, the 30-attempt sync wait) and the `step_sweep_stale_offers` arms ahead of the wallet arm in Step 8 (recovery-mode GUI pause, watchdog fallback, cancel-all drain), which the breaker never gated.
  - **(f) A per-heartbeat skip sets no gauge reason.** `xop_posting_gate{reason="wallet_circuit"}` (and the aggregate `xop_posting_gated`) reflects only an OPEN breaker, while a skip also bypasses the GUI-pause and breaker-pause TTL sweeps, so `gui/services/venue_control.py` `_NO_DRAIN_GATES` can read "draining" for a heartbeat that did not drain.
  - **(g) The probe and the gate disagree about what counts as an answer.** The gate treats `success=false` and HTTP errors as answers; the breaker's probe in `poll_loop_coro` treats any throw as "still unreachable", so an answering-but-unsynced wallet keeps an open breaker open.
  - **(h) Periodic pruning starves and is blind to never-broadcast rows.** The shared `last_stuck_tx_prune_block_` cooldown starves wallets later in the pair loop, and `pending_change` counts only mempool transactions, so a never-broadcast row is reachable only by the startup scan.
...
- **Why most downtime fills are caught.** Poll-throttle state (`fill_poll_pending_counts_`) lives in memory, so after a restart `fill_poll_due` is true for every offer `startup_reconcile` kept tracked (CONFIRMED ones included) once it is `detect_fills_min_age_blocks` (default 2) old. A settled offer is detected on the first heartbeat, and its fill either holds the gate for `confirmation_depth_blocks` (default 6) or is applied by Step 2 before Step 11 runs.
- **Windows that remain** (PR #154 review; analysis only, none reproduced live):
  1. **Swallowed poll failure.** `detect_fills` only logs a failed `get_offer`. If it keeps failing for one offer through the grace period while balance reads succeed, the one-shot runs first.
  2. **Same-heartbeat ordering.** Step 2 (`step_process_fills`) polls, Step 8 (`step_manage_offers`) then caches the wallet balance stamped `as_of_block = block_height`, and Step 11 reconciles against that snapshot. A settlement the wallet processes after an offer's last poll but before Step 8's read is in the snapshot but not yet detected. The gap is seconds for an offer in striking distance, up to `detect_fills_backoff_interval` (default 3) heartbeats for a backed-off one, and 2 blocks for a fresh one.
  3. **Reverse direction.** Set-to-wallet can also REVERT a fill Step 2 already applied, when the snapshot predates the fill's settlement. Step 11 accepts a snapshot up to `kBalanceMaxAgeBlocks` (10) old and Step 2 applies a fill only after 6 confirmations, so this needs Step 8 skipped on every heartbeat since the fill settled.
  - When any of them fires, the error lasts until the next restart, skews Step 7/8 sizing through `inventory_ratio`, and can raise a false `quote leg(s)` ExposureBreach alert on a later bid.
- **Status:** `[ ]` -- OPEN. Gating on the fill poll alone does NOT close window 2: the balance has to be read BEFORE a complete poll. Suggested shape (C++ follow-up):
  1. **Pre-poll snapshot.** While any tracked asset is unreconciled, read `get_wallet_balance` for each such asset at the top of the heartbeat, before `step_process_fills`, into a member such as `one_shot_pre_poll_balances_`. Zero RPCs once every asset is reconciled.
  2. **Poll completeness.** `OfferManager` records whether the last `detect_fills` polled every tracked offer successfully (`polled_ids.size() == pending_map.size()`, which already excludes age/backoff skips and failed `get_offer` calls) or had none to poll, and exposes `last_fill_poll_complete()`.
  3. **A pure predicate for ctest**, e.g. `one_shot_reconcile_ready(facts)` in `cpp/include/xop/accounting/reconcile_gate.hpp`: pre-poll snapshot taken this heartbeat AND poll complete this heartbeat AND fill buffer empty AND grace elapsed AND `pending_change == 0`. Step 11 then reconciles from the pre-poll snapshot instead of `cached_wallet_balances_`. One test per conjunct, each mutation-checked; the engine wiring stays S36.
  4. **Why it closes windows 1-3.** A settlement inside a snapshot taken before a complete poll is seen by that poll, so it sits in the buffer (holding the gate) or Step 2 has already applied it. Any fill Step 2 applies settled at least 6 blocks earlier, so it is already in that heartbeat's snapshot. Residual, to document: the wallet's own lag between coin state and trade status.
  5. **Alternative, unverified:** idempotent application, skipping legs for an asset reconciled at a snapshot height at or above the fill's `block_height` (the wallet's `confirmed_at_index`). That needs the snapshot stamped with the wallet's synced height, not the engine's `block_height`.

### S50: Avellaneda sizes the bid from TOTAL holdings, so every XCH-base pair stops bidding above q_max XCH
- **Files:** `cpp/src/strategy/avellaneda.cpp:268-270` (sizing), `cpp/src/engine.cpp` Step 4 (`q` from `net_inventory`), `cpp/src/risk/inventory.cpp:273-277` vs `cpp/include/xop/risk/inventory.hpp:198-201` (contract mismatch), `cpp/src/engine.cpp` Step 5 (`double q_max = config_.strategy.q_max;`)
- **Found 2026-09-13 while fixing the Step 6 attribution (branch `fix/step6-limit-attribution`):** the strategy sizes `bid = q_max*max(0, 1-q/q_max)` and `ask = q_max*max(0, 1+q/q_max)` with `q = net_inventory(base) / base_mojos_per_unit`. `net_inventory` returns `total_quantity` -- total holdings, seeded from the wallet at startup -- although `inventory.hpp:198-201` documents a signed net position. With 24.569719 XCH in `inventory_state` (2026-09-13 04:34) against `q_max: 20` (config.yaml:163), every XCH-base pair's strategy bid is 0 and its ask is 44.569719 XCH before any risk limit runs.
- **Step 5 ignores `q_max_override`.** It reads the global `strategy.q_max`, while the strategies are built with `effective_q_max()` (the pair override, else the global). Aligning Step 5 changes quoting, so it belongs with this redesign.
- **Status:** `[ ]` -- OPEN, operator decision: whether q should be a net position against a target rather than total holdings, whether q_max should scale with the wallet, and which of `net_inventory`'s doc and code is the intended contract.

### S51: a strategy-zeroed side plus a limit-zeroed side skips Step 7 and the deploy-idle floor
- **Files:** `cpp/src/engine.cpp` Step 6 (`evaluate_limits` reports no quote, so `quote_valid = false`), Step 7 `step_generate_ladder` (`if (!pcs.quote_valid) continue;`) and its deploy-idle floor
- **Found 2026-09-13 (branch `fix/step6-limit-attribution`):** XCH/BYC's bid is 0 from the strategy (S50) and its ask is zeroed by the single-CAT cap's full block: BYC holds 0.506-0.509 of the portfolio, 0.006-0.009 past the 0.500 point where the taper reaches 0 (`single_cat_cap_pct: 0.25` x the 2.0 full-block multiple). Both sizes are zero, so the pair skips Step 7 and the deploy-idle floor never runs: XCH/BYC posts nothing. XCH/DBX's ask survives the limits, so its strategy-zero bid does reach Step 7 and is floored to 1 XCH (`Step 7: XCH/DBX bid pool floored 0 -> 1000000000000 base mojos`, 1,160 lines in the 10.6 h of engine.log to 2026-09-13 04:45).
...
### S52: the cross-stable arb books an unmeasurable stable-pair spread as a FREE return leg
- **Files:** `cpp/src/engine.cpp:13260` (`if (ssnap.spread_bps > 0.0) stable_spread_bps = ...`), `cpp/src/engine.cpp:13377` (`return_leg_cost_bps = 0.5 * stable_spread_bps`)
- **Issue:** `stable_spread_bps` starts at 0 and is overwritten only by a POSITIVE snapshot spread. Whenever the stable pair's spread cannot be measured -- a one-sided, crossed or locked book, or a raw-ticker heartbeat (0 both before and after PR #159) -- the return leg therefore costs nothing. The comment that introduced the term (`engine.cpp:13247-13253`) prices that leg at 145-760 bps, half of a 290-1519 bps BYC/wUSDC.b spread, and calls its omission the single largest error in the strategy's economics; `:13373-13376` calls it the dominant term. Net edge is overstated by the largest cost exactly when the book is least observable.
- **Pre-existing.** Found in passing during PR #159 review; that PR changes nothing here.
- **Status:** `[ ]` -- OPEN. Candidate: when the stable spread is unmeasurable, refuse the take or charge a conservative floor instead of assuming a free return leg.

### S53: Dexie's `last` under CAT keys looks reciprocal -- BYC/wUSDC.b may ingest 0.48 instead of ~2.083 (PLAUSIBLE, unverified)
- **Files:** `cpp/src/rpc/dexie_client.cpp:698` (`orient_market_ticker` Case B passes `price_last` through), `cpp/src/engine.cpp:4286` (`ingest_dexie(..., price_last, ...)`), `cpp/src/engine.cpp:4290` (`price_last > 0` marks the pair's data valid), `cpp/src/execution/market_data.cpp:635` (`dex_last_trade`), `:1392` (compute_mid's last-trade fallback), `:1925` (`check_arbitrage`'s)
- **Evidence** (a `GET /v1/markets` response saved 2026-09-13 during PR #159, not committed): in all **4** CAT-keyed listings that carry prices, `last` lies OUTSIDE the listing's own [sell0, buy0] and `1/last` lies INSIDE it.
  - XCH under wUSDC: [1.0075, 95], last 0.5833, 1/last 1.7143
  - XCH under wUSDC.b: [1.8293, 2.5], last 0.4828, 1/last 2.0714
  - BYC under wUSDC.b: [0.9524, 373.97], last 0.48, 1/last 2.0833
  - XCH under BYC: [1.451, 3.3333], last 0.5, 1/last 2.0 -- and that 0.5 is exactly the `xch`-keyed BYC listing's `last`, which is XCH per BYC
  - Under `xch`, by contrast, `last` is inside for 70 of 96 listings and `1/last` for only 2.
- **Issue:** if CAT-keyed `last` is served in the reciprocal orientation, Case B's pass-through is wrong for `last` alone (bid and ask are verified), and the configured BYC/wUSDC.b pair ingests 0.48 where ~2.083 is meant. Four listings is a small sample, and nothing is checked against a settled trade. PR #159 does not change it: Case B passed `last` through before and after.
- **Status:** `[ ]` -- OPEN (PLAUSIBLE). Verify against a settled BYC/wUSDC.b trade (`/v1/offers?status=4`) before orienting `last` by key.

### S54: fair-value inputs have provenance (PR #159 Gate 1) but no recency
- **Files:** `cpp/src/execution/market_data.cpp:972` (`get_fair_value_inputs`), `:2530-2532` (`dex_print_age`, advanced only inside `ingest_competing_offers`), `:2508` (`ob_updated_at`), `cpp/src/engine.cpp:4556` (the per-pair catch)
- **Issue:** Gate 1 requires `bbo_from_filtered_book` but not `ob_updated_at` recency. A pair gets NO ingest on a heartbeat when `get_ticker` throws -- the per-pair catch skips its offers fetch too -- or returns nullopt while the offers fetch throws. The last filtered BBO then keeps its provenance and stays a `has_book` observation (and keeps publishing its `spread_bps`), and `print_age` cannot age it, because it advances only when a filtered ingest runs. Through a Dexie outage the fair-value solve and the quote-centre blend therefore keep the pre-outage book at its old weight.
...
### S66: an offer Dexie refuses for too many input coins rests unlisted until the ordinary cancel reaches it
- **Files:** `cpp/src/execution/offer_manager.cpp` (`submit_to_dexie`, `note_dexie_too_many_inputs`, `create_offer_min_coin`), `cpp/include/xop/execution/offer_min_input_coin.hpp`
- **Found 2026-09-19 (min-input-coin PR):** Dexie answers HTTP 400 "Too many input coins" for an offer built from more than about 125-132 inputs. The offer still exists in the wallet, so it locks its coins and can be taken by anyone holding the offer file, but no aggregator lists it. The one posted at 14:42:50 on 2026-09-19 (XCH/DBX bid tier 4, 62,228 characters) stayed that way until the ordinary cancel at 18:23:01. `engine.log` holds 13 such refusals between 2026-09-10 and 2026-09-19.
- **What the PR does:** `strategy.offer_min_input_coin_frac` (default 0.01) keeps dust out of the selection **on the first create for each offer**, so an offer built by that attempt needs at most 100 CAT inputs (the CAT leg only — the XCH fee coin is a separate selection this CAT-scaled floor does not bound), and a refusal logs one `[dexie-too-many-inputs]` warning. It adds no auto-cancel. The no-floor fallback is the exception, and it is what remains open below.
- **What is left:** the floor has a fallback. When the coins at or above it cannot cover the amount, the create is retried once without it, and that retry can build the same dust-funded offer again. Two candidates, neither decided: cancel an offer at once when Dexie refuses it for this reason (a fee-paying cancel per occurrence, and the tier stays empty until the dust is combined), or skip the tier instead of retrying without the floor (no offer, no fee, the same empty tier). Either needs the operator to choose between a resting unlisted offer and no offer.
- **Status:** `[ ]` -- OPEN. Operator decision. Combining the dust (`chia wallet coins combine`) removes the cause either way.
### S67: the blockchain fee was an open loop -- nothing checked whether our own spends confirmed
- **Files:** `cpp/include/xop/strategy/fee_controller.hpp` (new: the controller, the cost model, the budget rule, the tickets), `cpp/include/xop/rpc/node_requests.hpp` (new: the `get_fee_estimate` payload and reply, the mempool fields of `get_blockchain_state`), `cpp/include/xop/execution/fee_feedback.hpp` (new: `sent_to`), `cpp/src/strategy/fee_tracker.cpp`, `cpp/src/engine.cpp` (`fee_feedback_*`, Step 1, Step 2's verdict, Step 8), `cpp/src/execution/offer_manager.cpp` (`cancel_fee_for`), `tests/test_fee_controller_wiring.py`
- **Found live 2026-09-19/20 (v0.10.24):** Chia blocks ran ~97% full for a week. At capacity the node admits a spend only at >= 5 mojos per unit of CLVM cost (chia 2.7.4 `mempool_manager.py`, `nonzero_fee_minimum_fpc`) and refuses the rest with `INVALID_FEE_TOO_CLOSE_TO_ZERO` (48 distinct spend bundles refused in ten days, 3,230 wallet log lines). The engine's fee was `clamp(get_fee_estimate for spend_type send_xch_transaction, min, max)`: a 9.4M-cost XCH send, paid for CAT cancels measured at 42.05-42.26M and takes at 92.6M-212M. Nothing fed back whether our spends got in, so `pending_change` persisted and Step 8 force-deleted every unconfirmed wallet transaction 10-12 times a day. Raising `fees.min_fee_mojos` from 5,000 to 15,000,000 did not stop it.
- **Shipped OFF (`fees.controller_enabled: false`, `fees.cost_aware_estimate: false`).** With both off every fee is what v0.10.24 paid. The design, the evidence and every rule are in the header; the measured behaviour against a simulated hidden floor is in `cpp/tests/test_fee_controller.cpp` (12 actions to cross a 14x gap with no node; x1.05 mean overpayment at a fixed floor; 8 actions after a 10x jump; 0 with the node's admission floor).
- **Deliberately not done:** controller state is not persisted (the repo has no key-value table; the fee window itself is not persisted either), so a restart starts from level 0 plus the node's floor. Take BOOKING is untouched (S44/S48): the sweep only reads a pending take's status. A probe is tested on every spend submitted while it is in flight (two or three), not on one. A take's cost is one configured number, not a function of the maker's coin count. The bulk `cancel_offers` sweep still pays the single dynamic fee.
- **Review round 3 (2026-09-20), three confirmed findings, all fixed on the branch:** (1) the recommended `daily_budget_mojos` was smaller than the code's own cancel reserve -- at full-mempool prices 25 CAT cancels is 5,816,250,000 against a documented bottom of 5,000,000,000 -- so the reserve is now capped at half the budget AND the range is derived (`recommended_window_budget`: >= 30,327,171,874 at the shipped costs and `fee_window_blocks` 1662), logged at startup and quoted in `config.example.yaml` from the same computation; (2) an exhausted budget degraded CANCELS to `min_fee_mojos`, 0.35 mojos/cost against the node's 5, which is the stuck-cancel-then-force-delete failure the whole PR exists to remove -- a cancel or a take is now paid in full and the overrun reported (`FeeBudgetUnfunded`); (3) the on-target rule compared `level_of(fee paid)` with the raw level, so below the `min_fee` pin -- which is where the controller STARTS -- no confirmation could ever reach `on_target()`: `probing_` latched at the first probe after boot and the probe-down schedule was dead. It now compares both sides through the same clamp (`Controller::submitted_level_of`), and `Observation` carries its `ActionClass`.
- **Review round 8 (2026-09-21), five findings, all in comments/log lines/doc/test names rather than in the type-boundary work:** (1) `FeeBudgetBound` latched and cleared on `BudgetedFee::bound` ("the budget lowered the fee"), which is false whenever the attached fee is already pinned at `min_fee_mojos` -- every level at or below `log2(42.3/21) = 1.010`, the bottom 39% of the live band including the BOOT level -- so an exhausted budget never alerted there and an open episode was cleared by a quote from an empty window, logging `no longer binds (headroom 0 mojos)`; the latch is now `BudgetedFee::allowance < desired` on both edges (CODE fix); (2) `FeeBudgetUnfunded`'s clear branch never compared the fee paid with the headroom, so any accepted spend below its class's last quote ended the episode however far over the headroom it was (CODE fix: it now ends only when the spend fits the headroom); (3) the cancel reserve was documented as a bound on what attached fees may spend, which it is not -- `min_fee_mojos` overrides it, `should_post_offer` no longer refuses on budget grounds, and fees are booked per offer POSTED (DOC fix, in the header, `config.hpp`, `config.example.yaml`, CHANGELOG and the PR body); (4) a test named `ABatchOfAttachedFeesCannotSpendTheCancelReserve` demonstrated the opposite (renamed, and the overrun is now asserted); (5) the Reachability doc and four operator documents said the predicate was `< 5` and named two affected classes where the code uses `< 5 x ff_margin` and flags three (DOC fix; the code was right).
- **Review round 8 follow-up, CONFIRMED and NOT FIXED -- the bulk cancel sweep is still degraded.** `OfferManager::cancel_all` (offer_manager.cpp:1540) passes `current_fee_mojos_` to `cancel_offers_charged` as the wallet's `batch_fee`, and `current_fee_mojos_` is what `Engine::set_dynamic_fee` last wrote (engine.cpp:11242): the budget-shaped OFFER-ATTACHED fee. So with the controller on and the budget exhausted, a stop/shutdown Cancel All goes out at `fees.min_fee_mojos` -- 0.35 mojos/cost on a CAT cancel -- while `cancel_fee_for` prices every PER-OFFER cancel correctly from the class-aware priority fees. This makes the PR's blanket "a cancel or a take is never degraded" false for exactly one path. Not fixed on this branch: it changes the stop/shutdown sweep's behaviour and wants its own review. The fix is one line -- pass `std::max(cancel_fee_xch_mojos_, cancel_fee_cat_mojos_)` while `cancel_fees_active_`, the same "an unclassifiable offer errs toward getting in" rule `cancel_fee_for` already applies -- plus a source-scan pin, since nothing constructs an `OfferManager` in `xop_tests`. **It must land before `fees.controller_enabled: true`.**
- **Merge gate (2026-09-21), one finding, fixed on the branch:** the PR introduced `xop::to_mojo_saturating()` at FIFTEEN `uint64` -> `Mojo` fee conversions and pinned NONE of them. Measured on the four-way merged tree: dropping the wrapper from each of the four `CoinLockLedger` fee arguments in `offer_manager.cpp` was caught by nothing -- MSVC `/W4 /WX` is clean (same-size conversion, `-Wconversion` absent from both toolchains), the C++ suite passes (nothing constructs an `OfferManager`), and no scan mentioned the helper. #162's literal text pin had been an accidental guard and was rightly loosened. Six assertions in `tests/test_fee_controller_wiring.py` now pin it: the ledger fee argument POSITIONALLY (so #162's third `min_coin` argument cannot break it), the ternary/assignment/bind/ledger-leg shapes individually, a refusal of any bare narrowing cast on a fee, and an exact per-file census. Mutation: all fifteen sites unwrapped one at a time, each reddening exactly its targeted test plus the census, 16/16 predicted = actual. **Guard erosion, not a live defect** -- the narrowing needs a fee above 2^63 mojos (~9.2M XCH) against a `max_fee_mojos` clamp.
- **Review round 9 (2026-09-21), four confirmed, two CODE and two CLAIM:** (1) CODE -- `sent_to` is PER PEER, not a timeline, so `back()` was wrong both ways. Measured on the live `debug.log` (8 files, 38,250 non-empty lists): 8,289 are multi-peer, and 341 carry a fee refusal that is not last and were dropped. The parser (`sent_to_reports_fee_rejection`) now scans the whole array, suppresses on any SUCCESS/PENDING, and stops on an unreadable tuple rather than skipping it. Residual stated in the header: `filter_ok_mempool_status` strips the OK tuples on the resend tick, so a spend in an accepting peer's mempool is indistinguishable afterwards -- zero of the 38,250 live lists ever carried a SUCCESS, which is that filter seen from outside. (2) CODE -- `cancel_fees_paid` booked INTENT: it re-derived each fee from `cancel_fee_for()` even when the cancel fell through to `emergency_cancel` at a halved tier, at fee 0, or as a local-only cancel spending nothing. `OfferManager::take_cancel_fees_accepted()` carries the accepted fee out instead (memory: `taker-fills-booked-at-submit`, third instance on this PR); legacy branch byte-identical. (3) CLAIM -- "a refused row is reported once" is not an invariant: the capacity test runs before the insert and clears the whole set. (4) CLAIM -- class-aware cancel fees are wired in Step 8, which runs AFTER `startup_reconcile`, so the bulk sweep is NOT the only cancel that misses them; boot reconciliation cancels (incl. `OrphanDisposition::Unknown`, i.e. any resting offer on a disabled pair) pay the raw constructor fee. **Take-cost guidance corrected:** `max_fee_mojos: 700000000` covers only the MODELLED take (5.5 x 125,000,000); the measured take cost reaches 212,112,758, needing 1,166,620,169, and the cap alone cannot fix it because the fee is `rate x controller_cost_take` -- pricing the largest measured take needs `controller_cost_take: 212112758` AND `max_fee_mojos >= 1166620169`.
- **Status:** `[~]` -- BUILT, OFF. Staged enable is in the PR body: `cost_aware_estimate` first, then `controller_enabled` with `max_fee_mojos` raised (the startup log names each of the THREE classes the live 100M cannot get into a full mempool: offer-attached, CAT cancel and take) and `daily_budget_mojos` raised from the live 10,000,000,000 to at least 30,327,171,874. **At the live budget the two fixed alerts are the expected steady state, not a corner: enabling with 10,000,000,000 means the budget is exhausted almost at once, and before round 8 the operator would have seen neither alert.**

### S68: Step 8's force-delete counter advances once per PAIR per heartbeat, so "12 blocks (~10 min)" fires after a median of 176 seconds
- **Files:** `cpp/src/engine.cpp` (`step_manage_offers`: `bool saw_pending_change` is declared inside the per-pair block, engine.cpp:11867, and guards `++consecutive_pending_blocks_`, engine.cpp:11990), `cpp/include/xop/engine.hpp` (`kForceDeletePendingBlocks{12};  // ~10 min`, engine.hpp:2573)
- **Found 2026-09-20 while measuring the fee controller's target delay.** The counter is documented as "consecutive heartbeat blocks where pending_change > 0 was seen for any wallet". The flag that limits it to one increment is scoped to ONE PAIR's iteration, so with several enabled pairs sharing the XCH wallet it advances several times per heartbeat. Measured over the last 23 force-deletes in the live log: a median of 176 s (min 106 s, max 415 s) from the first `has pending_change=` line of the run to `force-deleting ALL`, a median of 6 engine cycles, against the ~10 minutes the constant's comment promises.
- **Why it matters:** `delete_unconfirmed_transactions` is wallet-wide. At ~3 minutes it fires inside the normal tail of a healthy confirmation (measured p90 up to 13 peak heights = 4 min) and takes unrelated unconfirmed spends with it. It is part of why 10-12 a day were seen, alongside the fee.
- **Not fixed in the S67 PR:** it changes behaviour with every flag off, and the right threshold wants an operator decision (count heartbeats, or wall-clock seconds).
- **Status:** `[ ]` -- OPEN. Move the flag out of the pair loop (one increment per heartbeat) and re-derive the threshold from peak heights; pin with a source scan, since nothing constructs an Engine.

### S69: the fee budget books offer-attached fees at POST though they are paid only on a fill, and its "24 h" window is 8.7 hours
- **Files:** `cpp/src/engine.cpp` (`fee_tracker_->record_fee(static_cast<std::uint64_t>(posted) * recommended_fee, ...)`, engine.cpp:13765), `cpp/include/xop/config.hpp` (`fee_window_blocks{1662}`, "~24 h at 52 s/block", config.hpp:2123)
- **Found 2026-09-20 (S67).** Two separate things make `fees.daily_budget_mojos` mean something other than its name. (1) A fee attached to a posted offer sits inside the offer and is paid by the combined bundle only if a taker fills it; a cancelled offer never pays it. Step 8 records `posted x fee` against the budget anyway: about 120 posts a day of phantom spend. (2) `fee_window_blocks` is compared against PEAK heights (18.75 s), not transaction blocks (52 s), so 1662 is 8.66 hours: the "daily" budget is a per-8.7-hour budget (memory: block-cadence-conventions).
- **Why it matters more with S67 on:** the controller can raise the attached fee, so the phantom spend grows and the budget binds early. S67 contains it rather than fixing it: offer-attached fees may spend only what is above a reserve of `fees.controller_budget_reserve_cancels` CAT cancels (itself capped at half the budget, review round 3), an exhausted budget degrades that fee to `min_fee_mojos` instead of returning 0 (which made Step 8 skip cancels too), and one `FeeBudgetBound` alert says so. A PER-OFFER cancel or take is never degraded -- it is paid over budget and reported with `FeeBudgetUnfunded` -- because `min_fee_mojos` on a 42.3M-cost cancel is 14x below what a full mempool admits. (The BULK stop/shutdown sweep is the one exception and is NOT fixed; see the round-8 follow-up under S67.)
- **The phantom booking is what makes the budget so large.** The derived sizing S67 now prints at startup (`strategy::fee::full_mempool_window_cost`) counts an attached fee for every offer POSTED because `record_fee` does: 152 x 115,500,000 = 17,556,000,000 of the 42,042,000,000 daily total, i.e. **42% of the recommended budget is spend that never happens**. Fixing this item roughly halves the number config.example.yaml asks operators to set.
- **Status:** `[ ]` -- OPEN. Book an attached fee when its offer FILLS (the maker fill path knows `fee_mojos`), and either rename the key or default the window to 4,608. When it lands, re-derive `DailyActions` in `fee_controller.hpp` (drop `offers_posted`) and the example file's arithmetic with it.
### S70: the hard TTL pays a fee to retire an offer the chain would retire for free
- **Files:** `cpp/include/xop/execution/offer_expiry.hpp` (`age_limit_cancel_applies`, `trade_record_max_time`, `expired_retire_clock_height`, `expired_at_depth`, `decide_expired_retire`), `cpp/src/execution/offer_manager.cpp` (`classify_tier_staleness`, `cancel_stale`, `retire_expired_offers`, `detect_fills` backfill), `cpp/src/engine.cpp` (Step 8, below the sync gate), `cpp/include/xop/rpc/wallet_requests.hpp` (`get_timestamp_for_height`)
- **Found 2026-09-20:** `ttl_expired` was 402 of the 1,254 cancels in the 14 days to block 9,319,413, at an average age of 992 blocks -- correctly priced offers cancelled and reposted at nearly the same price. Offers posted since the operator enabled `offer_expiry_secs: 86400` on 2026-09-20 carry a verified `max_time` (#150), so the chain already bounds their life.
- **Wallet facts, read from chia-blockchain 2.7.4 (98aba3d1):** the wallet never retires an expired trade (nothing reads `valid_times` after it is stored); `TradeManager.get_locked_coins` keeps its coins locked with no time test; `cancel_offer` with `secure=false` sets it CANCELLED before any transaction is built and that releases them; and `ASSERT_BEFORE_SECONDS_ABSOLUTE` is tested against the PREVIOUS TRANSACTION BLOCK's timestamp, so the clock that says "expired" has to be the chain's. Dexie marks such an offer status 6 (expired) at `date_expiry` with no spend.
- **Done (default OFF):** `strategy.ttl_cancel_mode: expire` skips the hard-TTL cancel and the stuck pass for an offer with a verified expiry, and retires it with a free local cancel once a chain clock read 32 blocks BELOW `get_height_info`'s finished-sync height (`get_timestamp_for_height`) is at or past `max_time`, the wallet still says PENDING_ACCEPT, and its record repeats the tracked `max_time`. The depth is the reorg allowance: the first draft used a 600 s margin at the tip, which review on #164 showed the tip block alone can satisfy after a slow patch. The offer_log row is still completed only by the wallet verdict (`expired_onchain` is a submit-time reason).
- **Corrected 2026-09-21 (review #164, verified against the 2.7.4 source):** that clock is NOT "the wallet's own". Only the HEIGHT is anchored to the wallet's processed chain; `WalletNode.get_timestamp_for_height` (wallet_node.py:1299-1304) returns the first answer from any connected full-node peer, called with `expected_header_hash=None`, so on a cache miss the only validation is `len(response)==1` and `block.height==request_height` (:1258-1266) -- no signature, PoSpace, VDF or consensus. The 32-block depth bounds reorgs only and buys nothing against a lying peer. `ChainClockTrust` / `rpc::census_full_node_peers` now gate the pass on every full-node peer being on this host (`get_connections`), read before the clock and again after it. **Precondition for enabling `expire`:** the wallet reaches the chain only through a local full node. It lapses when that node is down (`on_disconnect` -> `initialize_wallet_peers`, wallet_node.py:838-841, then untrusted sync at :1481-1499) and in the transient after it returns (the localhost peer sits in the `trusted` bucket behind already-`synced` strangers, :1203-1228). The engine's `get_sync_status` gate closes neither.
- **Not done:** `offer_log` has no expiry column, so a restart recovers each offer's expiry from the first `get_offer` poll (Step 2 runs before Step 8); an offer the poll backoff skips on that first heartbeat, and is already past the hard TTL, is cancelled the old way. The stopped-engine sweep (`ttl_while_stopped`) is deliberately unchanged. #150's startup floor (`offer_expiry_secs` > hard TTL x 2) is also unchanged, although under `expire` the hard TTL no longer applies to the offers it protects: it keeps the expiry at ~23 h or more at `offer_ttl_blocks: 400`, and an offer the market has drifted away from rests that long when `price_cancel_mode: margin` is on as well. Relaxing the floor in `expire` mode trades that for a larger share of each offer's life spent expired-but-still-tracked (32 blocks, ~600 s, per life), and is the operator's call. The GUI pairs table sizes its resting-offer window from the expiry in this mode (`book_window_ttl_blocks`); no other GUI surface knows the three keys.
- **Status:** `[x]` -- DONE behind the switch; enable per the PR's staged procedure.

### S71: Step 8's two exposure checks disagree the moment an offer locks a coin
- **Files:** `cpp/include/xop/execution/exposure_gate.hpp` (`decide_exposure`, `resting_spend_on_asset`, `exposure_cancel_candidate`), `cpp/src/engine.cpp` (the resting-offer check and the pre-post projection)
- **Found 2026-09-20:** `exposure_floor_rebalance` was 528 cancels (42%) at an average age of 27 blocks, 518 of them on 2026-09-13/14, all XCH/DBX. Reconstructed from engine.log 2026-09-13 00:35-00:39: the pre-post projection approved an ask on `spendable - (pending + new)`; the wallet funded the 1-XCH ask from a 1.2079-XCH coin and locked the whole coin; the resting-offer check then saw the same six XCH of asks against a spendable 1.2079 lower and cancelled the offer 24 s after posting it; the cancel's change confirmed and the loop ran again.
- **Why:** `spendable_balance` in chia 2.7.4 already excludes every coin a resting offer locks (`get_spendable_coins_for_wallet` subtracts `get_locked_coins()`), so `spendable - pending` counts a resting offer twice and moves by the size of the coin the wallet happened to pick.
- **Done (default OFF):** `strategy.exposure_rule: unified` gives both sites one verdict from `unconfirmed_wallet_balance` against every resting offer that spends the asset (all pairs), keeps a spendable test only for funding new tiers, cancels resting offers only below `reserve x (1 - exposure_cancel_hysteresis_pct)` and never one younger than `exposure_cancel_min_age_blocks`, and suppresses the next post instead. `legacy` runs through the same function and is pinned equal to the old predicate on a grid of inputs.
- **Added under review #164, and it is operator-visible:** a tracked offer whose pair `find_pair_config` cannot resolve (an adopted `UNKNOWN` record, or a pair since REMOVED from the file -- a merely DISABLED pair is still mapped) is recorded as UNQUANTIFIABLE (`RestingSpend::pair_unmapped`, `has_unmapped_live_claim`) rather than dropped, because `owned` still counts the coins it locks. While such a claim is LIVE, `unified` refuses to ADD exposure on either side and never manufactures a `CancelResting` for it. That is not a reserve breach and does not borrow the reserve-breach line: it logs `exposure (unified) ... cannot be projected -- a LIVE resting offer's pair is not in this config`. The reload drain clears it on the next heartbeat.
- **Status:** `[x]` -- DONE behind the switch.

### S72: the selective refresh cancels for ladder drift, not for what a fill would earn
- **Files:** `cpp/include/xop/execution/cross_guard.hpp` (`classify_tier_refresh_margin`, `resting_edge_bps`, `margin_breach_reason`), `cpp/src/execution/offer_manager.cpp` (`classify_tier_staleness`), `cpp/src/engine.cpp` (Step 8's classify call), `cpp/tests/test_price_cancel_replay.cpp`, `scripts/gen_price_cancel_replay_fixture.py`
- **Found 2026-09-20:** `price_adverse` was 282 cancels (22%), median deviation 1.30%. The XCH/DBX tier-0 ask of 2026-09-10 went 78.69 -> 79.52 -> 78.70 -> 79.94 in three hours, one fee per leg.
- **Done (default OFF):** `strategy.price_cancel_mode: margin` cancels for price only when a fill at the resting price would earn less than `price_cancel_edge_retain` (default 0.5) x Step 7's own floor against BOTH of Step 7's centres -- the shifted ladder centre the floor is measured from and the fair-value centre before the A-S shift -- all threaded from `PairCycleState` (the shifted centre alone is the wrong frame for edge; the fair centre alone cancels what the pricer just posted on the side being shed, whenever the shift exceeds (1 - retain) x floor); crossed still outranks everything, `kMinRefreshAgeBlocks` still applies, the anchor override is off in this mode, and a missing reference falls back to the deviation zones.
- **Replay (recorded offers, open-loop, ladder-reconstructed centre/floor):** at the blocks the old rule cancelled, the margin rule would have made 82 of the 248 witnessed price cancels at retain 0.5 (122 at the literal 1.0). Over whole recorded lives it fires on 147 offers at 0.5 -- and on 347 at 1.0, MORE than the old rule's 282, because Step 7 posts tiers exactly at the floor. 7 of the fortnight's 15 fills rested at some block below half the posting floor.
- **Status:** `[x]` -- DONE behind the switch. The retain fraction is the operator's call; see S73.

### S73: Step 7's centre and floor are not persisted, so no replay of a price rule can be exact
- **Files:** `cpp/src/engine.cpp` (the snapshot batch, `DbSnapshot`), `cpp/src/database.cpp` (`snapshots`)
- **Found 2026-09-20 while replaying S72:** `snapshots.mid_price_mojos` is the published book mid, which sat 267 bps from Step 7's blended centre on XCH/DBX on 2026-09-14; `quote_mid_mojos`, `quote_fair_centre_mojos`, `quote_min_half_spread_bps` and the combined sigma are recorded nowhere. The S72 replay therefore reconstructs centre and floor from the innermost recorded ladder tiers, which is an upper bound at retain 1.0 and only an estimate below it.
- **Wanted:** four nullable columns on `snapshots` written from `PairCycleState`, then regenerate the S72 fixture from them and re-pin the counts. Also wanted once `margin` has run live for a week: the same census this PR opened with, by `cancel_reason`, to check the replay against reality before the default retain is argued about further.
- **Status:** `[ ]` -- OPEN.

### S78: an early `expire` retire has no local alarm -- but the wallet is not blind to one
- **Files:** `cpp/src/execution/offer_manager.cpp` (`retire_expired_offers`), `cpp/include/xop/execution/offer_expiry.hpp`, `cpp/src/engine.cpp` (Step 8; S14's escalation at `:21771-21794` already does the coin lookup), `cpp/include/xop/monitoring/on_chain_reconciler.hpp`, `cpp/include/xop/rpc/chia_rpc.hpp` (`ChiaFullNodeRPC`)
- **Found 2026-09-21 (review #164, round 3).** #164's CHANGELOG and operator playbook said "no LOCAL signal can report a retire that went wrong" and pointed the operator at Dexie as the sole detector. The justification -- `get_trades_by_coin` skips CANCELLED (`trade_manager.py:131-139`), so `coins_of_interest_farmed` never fires and no fill is booked -- supports only the narrower claim that the **trade record** is blind. Verified against chia 2.7.4: a take spends our maker coin, and the offer's requested payments are notarized payments to `p2_ph = await action_scope.get_puzzle_hash(...)`, an address the maker's OWN wallet derived (`trade_manager.py:500, 518`). A take therefore moves coin records and balances this bot already reads. The absolute was wrong; the narrow claim is now what #164 says.
- **Wanted, check 1 -- falsify the premise from THIS host's own full node. No attribution needed, and complete for the failure being watched.** The retire acts on one claim: `ts(H - kExpiredRetireDepthBlocks) >= max_time`. `ChiaFullNodeRPC::get_block_record_by_height` answers that directly from the node the bot already configures (live `full_node_host: localhost`, `full_node_port: 8555`), backtracking via `prev_transaction_block_height` when the height is not a transaction block -- verified read-only 2026-09-21 at peak 9,324,542, which returned `timestamp`, `header_hash` and `prev_transaction_block_height`. That node validated the block under consensus and is already inside this bot's trust boundary. If it disagrees with the wallet's peer, the retire was premature. This catches EVERY take, because chia_rs `check_time_locks` fails a take once the PREVIOUS transaction block's timestamp is at or past `max_time` and transaction-block timestamps do not decrease -- so no take after an honest retire is possible at all. Re-read it over a window after the retire too: a reorg deeper than the depth can falsify a premise that was true when it was read. It detects a premature RETIRE rather than a take; a true positive with no take is still the alarm worth having.
- **Wanted, check 2 -- identify an actual take, and resolve the attribution caveat.** Watching the maker coins alone is ambiguous: after a free retire the bot legitimately re-spends the freed coins. The discriminator is not "was the coin spent" but "did that spend pay us what the offer asked for". The machinery exists: `parse_coins_of_interest` -> `coin_names_for` -> `full_node_->get_coin_records_by_names(names, include_spent=true)` -> `classify_coin_records`. When a retired offer's maker coin turns up spent at height B, fetch `get_block_record_by_height(B).header_hash` and then `get_additions_and_removals(header_hash)` -- both already on `ChiaFullNodeRPC` -- and look for a coin of the offer's REQUESTED asset at one of our own puzzle hashes (`OnChainReconciler::run_full_reconciliation` already takes `our_puzzle_hashes`). A wallet re-spend creates no such payment. Caveat: for a CAT the payment coin carries the CAT outer puzzle wrapping our inner hash, so match tracked hashes, not a bare p2 hash.
- **Larger than the alarm, and deliberately NOT done in #164:** check 1 is a PRE-check, not only a detector. Reading the timestamp from our own node answers the NUMBER, which #164 concluded could not be checked and which the peer-provenance gate only works around by checking WHOSE answer it is. Requiring the local node to agree before retiring would keep `expire` safe in exactly the two states the gate currently closes by pausing (local node down; the transient after it returns). It is a real design change with its own failure modes -- the wallet's height and the node's chain are two views, and a `wallet_only` deployment has no such node -- so it needs its own PR, tests and mutation campaign.
- **Status:** `[ ]` -- OPEN. #164 implements none of this; its shipped detector is Dexie, and it says so.
### S74: a graceful stop could only CANCEL the book, so keeping offers across a restart meant a hard kill
- **Files:** `cpp/include/xop/util/stop_offers_policy.hpp` (the policy and its table, the drain decision and its budget), `cpp/include/xop/util/shutdown_flag.hpp` (the `offers=` line), `cpp/include/xop/execution/kept_book.hpp` (the keep line), `cpp/src/engine.cpp` (`Engine::shutdown`, `report_offers_kept_on_stop`, `watchdog_cancel_book`, `evaluate_shutdown_flag`, `on_new_block_coro`'s mid-cycle stop gate), `cpp/src/execution/offer_manager.cpp` (`PostingMark` around each `create_offer`), `cpp/src/config.cpp` (`engine.shutdown_offers`), `gui/stop_offers.py`, `gui/widgets/stop_engine_dialog.py`, `gui/widgets/main_window.py`, `gui/services/engine_bridge.py`, `gui/widgets/settings.py`
- **Operator request, 2026-09-20.** `Engine::shutdown()` cancelled the whole book on every graceful stop. For a week the GUI and the engine were hard-killed (TerminateProcess) whenever offers had to survive a restart, and with blocks ~97% full a cancelling stop leaves unconfirmed cancel spends and locked coins for the next start.
- **Fixed.** A stop carries a policy, `cancel` or `keep`: the stop request's `offers=` line, else `engine.shutdown_offers` (default `cancel`). `keep` sends no cancel, writes no cancel intent, changes no `offer_log` row, disarms the dead man's switch first, and logs what was left resting and until when. The GUI asks on Stop Trading and on window close; a session end or a signal never prompts.
- **Not covered by a C++ test:** the wiring in `engine.cpp` (S36). `tests/test_stop_keep_wiring.py` pins it over the source text -- lint-class, disclosed there. That scan **cannot read string literals**, so every operator-facing sentence the keep stop prints is built in `kept_book.hpp` and pinned by gtest instead; the residual gap is S77.
- **Status:** `[x]` this PR.

### S75: an offer past its on-chain expiry is cancelled with a PAID secure cancel, and boot does not say which restored offers have already expired
- **Files:** `cpp/src/execution/offer_manager.cpp` (`startup_reconcile` Phase 1, `classify_tier_staleness`, `cancel_stale`), `cpp/include/xop/execution/offer_expiry.hpp`
- **Found while verifying the start after a keep stop (S74), 2026-09-20.** The reference wallet never reacts to `max_time` passing: an untaken maker offer stays `PENDING_ACCEPT`, `get_all_offers` still lists it, and `get_locked_coins()` still counts its coins (chia-blockchain 2.7.4 `chia/wallet/trade_manager.py`: no `set_status` call depends on time, and `TradeStatus.EXPIRED` is referenced nowhere in `chia/wallet`). So after a stop longer than the expiry, `startup_reconcile` restores the offer as known and live.
- **Why it is not quoted as live for long:** the config floor (`expiry_outlasts_hard_ttl`) makes any expired offer at least 2x past the hard TTL, which `classify_tier_staleness` expires unconditionally on the pair's first Step 8 pass, and the STOPDRAIN sweep (`ttl_while_stopped`) does the same at the soft TTL when the engine boots paused. `cpp/tests/test_stop_offers_policy.cpp` (`KeptBookAfterRestart`) pins the argument. Until that pass the engine's view matches the wallet's: the coins ARE still locked.
- **That holds for `strategy.ttl_cancel_mode: cancel` only** (the default, and the only behaviour of any build before PR #164 added the key; there is no value spelled `age` -- #164 spells it `cancel | expire`). Under `expire`, `execution::age_limit_cancel_applies(expire_mode, verified_max_time)` is false for any offer carrying a verified expiry, so the hard-TTL cancel is skipped and the offer rests until `retire_expired_offers` frees it -- after its own `max_time`, plus the reorg depth. The bound a kept book really has is then the expiry (24 h at the live `offer_expiry_secs: 86400`), never the hard TTL, and the operator note says both. `KeptBookAfterRestart.UnderExpireModeTheBoundIsTheExpiryAndItOutlastsTheHardTtl` pins that the expiry is the longer of the two at every configuration the startup floor accepts.
- **What is wasteful:** the cancel is a SECURE one -- a fee and block space, with blocks ~97% full -- for an offer nobody can take. `cancel_offer(secure=false)` would release the coins at once for nothing. It is only safe when the offer is PROVABLY expired against chain time (the last transaction block's timestamp), not the host clock: a fast host clock would drop a still-takeable offer from the wallet's books, and a take on it would then be booked nowhere. `OfferManager` holds no full-node client today.
- **Also:** the wallet record carries `valid_times.max_time`, and Phase 1 has every known-live record in hand, but nothing reads it -- so neither the boot log nor State says which restored offers have already expired, and `PendingOffer` carries no on-chain expiry at all (S76).
- **Status:** `[ ]` -- OPEN.

### S76: State carries no on-chain expiry per offer, `engine.shutdown_offers` is read at startup only, a signal-delivered keep stop still cuts the rest of its cycle, and no stop policy can be guaranteed at an OS session end
- **Files:** `cpp/include/xop/types.hpp` (`PendingOffer`), `cpp/include/xop/execution/kept_book.hpp`, `gui/stop_offers.py` (`summarise_resting_offers`), `cpp/src/engine.cpp` (`check_config_reload_flag`), `gui/main.py`
- **Found while building S74, 2026-09-20.** Four limits, each stated where it bites rather than hidden:
  - **The keep line's expiry is exact only for offers this process posted.** `PendingOffer` has a wall-clock creation time for those; an offer restored from `offer_log` or adopted from the wallet has none, so the line gives a bound ("no later than now + expiry, if it carries the configured one") instead of a date. The GUI prompt estimates from `offer_log.created_at`, the row's insert time. Carrying the echo-verified `max_time` on `PendingOffer` (and in `offer_log`) would make both exact; it touches the three posting sites in `offer_manager.cpp` and needs a schema column, so it was left out of S74.
  - **`engine.shutdown_offers` is restart-required**, like every key the reload does not apply live. A Settings change made while the engine runs takes effect at the next engine start; until then an unanswered stop uses the value the engine booted with. The GUI prompt always sends an explicit policy, so this only affects stops nobody answers.
  - **A keep stop delivered by a SIGNAL can land inside a heartbeat cycle** (review of PR #165). `shutdown.flag` is read between cycles, so a GUI stop never does; Ctrl+C or SIGTERM can, while Step 8 is suspended in `post_quotes`. FIXED for the case that mattered: the keep stop waits (`util::keep_stop_drain_step`) while ONE `create_offer` is in flight and its offer is not yet in State, so a create the wallet is still answering lands there instead of becoming an orphan -- which `evaluate_orphan` CANCELS at the next boot when it is older than `orphan_max_adopt_age_blocks` (120 blocks, about 37 min at the peak-height cadence), adversely priced, or on a pair with no mid price. The mark is per create, not per ladder, and the budget is SIZED to that window (`util::keep_stop_drain_budget_ms`: the create's own retry ladder plus the Dexie submission that stands between the wallet's answer and the State write, 247 s with the shipped timeouts) -- the first draft marked a whole pair's ladder, up to 12 creates, under a flat 60 s budget that could expire with a create still outstanding. **(a) FIXED in review round 4** (Copilot 4058780337): `OfferManager` now takes a second, NON-cancelling predicate (`set_stop_creating_predicate`, wired to `stop_requested_` beside the S31 abort predicate, which cancels the offer that landed late and so cannot be used by a keep stop). Once a stop is latched no further create begins -- on the tier loop, the merged batch and the batch fallback alike -- so the drain waits for at most the ONE create already in flight and the 247 s budget is a real upper bound rather than a number sized for one create and spent on a ladder. **(a2) FIXED in the same round** (Copilot 4058780326): a create that fails with NO ANSWER releases its `PostingMark` at once -- correctly, since nothing is left for the io_context to wait for -- but a timeout is not a refusal, so `Engine::create_outcome_unknown_` records it (`rpc::request_possibly_submitted`) and the keep stop reports a possibly-untracked offer instead of clean success. What remains open: (b) a create still unanswered at the budget is abandoned, and the log says so at error; (b2) on THIS branch the merged batch's fallback still rebuilds a pair tier by tier after a possibly-submitted merged create, which can put a second set of offers on top of one the wallet already holds -- **#162 (open) fixes exactly that**, suppressing the fallback on the same evidence, so this is open only until that merges and is deliberately not duplicated here; (b3) nothing ever clears `create_outcome_unknown_`: a periodic `reconcile_offers` that adopts the wallet's phantom really does retire the doubt, and could; (c) a cancel or a take the cycle was awaiting when the stop landed is still cut -- the wallet settles it alone, boot adopts a PENDING_CANCEL record as `cancel_pending`, but a take completed that way is booked nowhere, exactly as after a hard kill. NEW ones are no longer started: the cycle now ends at a `stop_requested_` gate after Step 8, and Step 9f (the drift corrector, the engine's own taker) carries the same gate.
  - **An OS session end is best-effort whatever the policy says.** The GUI writes the request from `aboutToQuit` and waits up to 30 s, but Windows ends the GUI and its child engine within seconds. With `cancel` configured, the sweep may be cut short (the write-ahead intent in `uncancelled.txt` still lets the next start finish it); with `keep`, a kill is equivalent. The engine has no `CTRL_LOGOFF_EVENT` / `CTRL_SHUTDOWN_EVENT` handler.
- **Status:** `[ ]` -- OPEN.

### S77: the engine wiring scans strip string literals, so no test reads what the operator reads
- **Files:** `tests/test_stop_keep_wiring.py` (`_code`, `_no_comments`, `_literals`, `CLAIM_WORDING`), `tests/test_rpc_retry_wiring.py`, `tests/test_offer_status_write_sites.py`, `cpp/src/engine.cpp` (every `spdlog::` message in it)
- **Found in review of PR #165, 2026-09-21.** Nothing in `cpp/tests` can construct an `Engine` (S36), so the lint-class Python scans pin `engine.cpp` over its source text -- and `_code()` empties every string literal before matching, which is right for matching CODE and blind to every sentence the operator actually reads. That blindness shipped a defect through two review rounds: `report_offers_kept_on_stop` logged a flat *"No offer post was left unrecorded."* unconditionally, forty lines below `post_abandoned` and `create_outcome_unknown_`, the two facts computed to say the opposite -- and `post_abandoned` IMPLIES that branch, so the contradiction held on every stop that reached it. Both lines were pinned by the scan; neither pin could see the sentence.
- **Partly fixed in #165.** Every operator-facing sentence the keep stop prints now lives in `cpp/include/xop/execution/kept_book.hpp` (`describe_kept_book`, `describe_cut_cycle`, `describe_watchdog_disarm`), where gtest reads exactly what the operator reads, and the scan pins that the engine passes the real facts. Two guards were added: the mid-cycle branch may hold ONE literal and it must be a bare format shell (general -- any prose fails it), and a **vocabulary** check over the rest of the report.
- **What is still open, and why it is filed rather than done:** the vocabulary check catches a returning claim in the wording it returns in, not an arbitrary new one. The general property -- *no literal in `engine.cpp` may assert a safety property* -- needs every log message in that file moved behind a pure builder, or a per-function inventory of approved message text. `engine.cpp` is ~22,000 lines and that is its own PR.
- **Status:** `[ ]` -- OPEN. Next step: apply the kept_book.hpp pattern to `watchdog_cancel_book` and the S46 cancel-ladder lines, which make the same class of claim about cancels.
