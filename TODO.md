...
- **Status:** `[~]` -- BUILT, OFF. Staged enable is in the PR body: `cost_aware_estimate` first, then `controller_enabled` with `max_fee_mojos` raised (the startup log names the class the live 100M cannot get into a full mempool).
...
- **Why it matters more with S67 on:** the controller can raise the attached fee, so the phantom spend grows and the budget binds early. S67 contains it rather than fixing it: cancels and takes keep a reserve (`fees.controller_budget_reserve_cancels`), an exhausted budget degrades fees to `min_fee_mojos` instead of returning 0 (which made Step 8 skip cancels too), and one `FeeBudgetBound` alert says so.
- **Status:** `[ ]` -- OPEN. Book an attached fee when its offer FILLS (the maker fill path knows `fee_mojos`), and either rename the key or default the window to 4,608.
