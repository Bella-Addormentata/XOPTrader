...
// is racing INDEPENDENT stoppers with wall-clock budgets, and the engine does
// not get another cycle to make up for a slow one.
//
// HOW LONG THE SHUTDOWN AS A WHOLE MAY TAKE IS NOT ESTABLISHED, AND THIS
// HEADER DOES NOT ESTIMATE IT. Three separate attempts at that figure were
// made in this PR's history and each was shown wrong by the next review
// pass; the last was still unaccounted for close_connections() blocking on
// thread_pool_->join(). The retracted numbers are recorded in TODO.md S46
// rather than here, so that a grep of the source finds no such figure to
// mistake for a live claim.
//
// What is stated below is the SCHEDULE THIS POLICY KEEPS: when it sleeps,
// when it stops, and when it last asks the wallet. That is a property of
// this file, it is proved at compile time, and it is the only timing claim
// made here. Do not add a shutdown total, a worst case, or a margin against
// watchdog_stall_seconds to this header without first establishing one.
//
...
// so no retry length whatsoever helps them. Neither does any stopper that
// terminates the process before the ladder finishes: whatever the ladder had
// not yet cancelled is simply left live, and NOTHING RECORDS THAT IT WAS
// MEANT TO BE DEAD. That gap is real, it is not closed by this PR, and it is
// filed as its own TODO entry rather than papered over here -- an earlier
// version of this change tried to close it with a durable intent file and
// produced a fresh fail-open on each of three review passes.
//
// So: this header covers the FLAP, which is what the 2026-09-02 incident
// actually was. It does not cover the kill.
//
...
// enumerator is `Unknown`, and the caller must NAME THE STILL-LIVE OFFER IDS
// in its alert. Stopping is allowed to be the default; stopping SILENTLY is
// not reachable.
//
...
struct CancelRetryConfig {
    ...
    ///
    /// This is NOT a bound on the shutdown's duration and this header does
    /// not claim it is. It bounds when new work may START. An RPC already in
    /// flight is not interrupted -- see the note on cancel_ids_admits() below.
...
/// time.
///
/// SLEEPING is all this bounds. It says nothing about how long an RPC already
/// in flight takes, and therefore nothing about the total duration of a
/// shutdown -- which is not established. See the header note.
[[nodiscard]] constexpr std::uint32_t
...
// READ THE ASSERTS BELOW WITH THEIR SCOPE ATTACHED. They prove a property of
// THIS SCHEDULE -- when the ladder would last ask the wallet IF IT IS ALLOWED
// TO RUN TO COMPLETION. They say nothing about whether it is allowed to. Any
// stopper that terminates the process earlier truncates the ladder wherever
// it happens to be, and the attempts beyond that point never happen. "85 s
// covers the measured-longest 80.9 s episode" is a true statement about this
// ladder and NOT a statement about any particular shutdown route; state it
// with that qualifier or not at all.
